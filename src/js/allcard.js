...
export async function Cards() {
    ...
    if (data.errors !== undefined) {
        ...
        let cards = level(data)//level
        let cards1 = card_1(data)
        let cards2 = card_2(data)
        let graph_Cards = graph_Card(data)
        // let graph_Card_1s = graph_Card_1(data)
...
function card_1(data) {
    ...
    Object.entries(data).forEach(([_, ele]) => {
        top_skills = ele.user[0].sklis.sort((a, b) => b.amount - a.amount).slice(0, 4);
...
function card_2(data) {
    ...
    Object.entries(data).forEach(c => {
        ...
        } else if (xp_total / 1000 >= 1) {
            xp_level.textContent = `${Math.round(xp_total / 1000)}KB`
        }
        else if (xp_total / 100000 >= 1) {
...
function DivsIndes(classes) {
    let divs = document.createElement("div")
    divs.classList = classes
    return divs
}
function spans(clas, cont) {
